#!/usr/bin/env python3
"""Generate weighted operation counts for the two-dimensional kernels."""
...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "flop_count_2d.pkl"
...
def generate_counts(max_order: int = 8) -> dict[str, dict[str, list[int]]]:
    ...
    x0, x1, y0, y1, c0, c1, ny0, ny1 = sp.symbols(
        "x0 x1 y0 y1 c0 c1 ny0 ny1"
    )
    x = sp.Matrix([x0, x1])
    y = sp.Matrix([y0, y1])
    c = sp.Matrix([c0, c1])
    ny = sp.Matrix([ny0, ny1])
    ...
    single_layer = sp.besselk(0, k * dist_line)
    double_layer = sum(single_layer.diff(y[i]) * ny[i] for i in range(2))
    kernels = {"S2d": single_layer, "D2d": double_layer}
