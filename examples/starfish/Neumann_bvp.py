...
def visualize_results(data):
    ...
    n_lam = len(lams)
    fig, axes = plt.subplots(2, n_lam, figsize=(10, 5.5), sharex=False, sharey='row')
    ...
    for i, lam in enumerate(lams):
        ...
        for j, tau in enumerate(taus):
            ...
            for n in nelements:
                rel_error = results[lam][tau][n]['error']  / np.max(results[lam][tau][n]['ref'])
        ...
        error_fit = [np.abs(results[lam][fit_tau][n]['error'] / np.max(results[lam][fit_tau][n]['ref'])) 
                    for n in nelements[1:6]]
        
        log_hmax = np.log10(hmax_fit)
        log_error = np.log10(error_fit)
        slope, intercept, r_value, p_value, std_err = stats.linregress(log_hmax, log_error)
    ...
    plt.suptitle(f'Interior Neumann Problem - $p={expn_order}$, $q={target_order + 1}$, $\\kappa={upsampling_factor}$')
    plt.tight_layout()
    plt.show()
...
if file_exists:
    ...
    def solve_neumann_system(geometry_data, lam, nelement, target_order, upsampling_factor, expn_order):
    ...
    def evaluate_solution(density_f, geometry_data, eval_targets_h, eval_targets_normals_h, 
        ...
        from sumpy.expansion.local import AsymptoticDividingLineTaylorExpansion
        asymexpn = AsymptoticDividingLineTaylorExpansion(base_knl, asym_knl, expn_order, tau=tau)
        ...
        eval_result = eval_mat @ density_f * np.exp(-lam * eval_expansion_radii_h * (1 - tau))
    ...
    for nelement in nelements: 
        ...
        for lam in lams:
            density, gmres_iters = solve_neumann_system(geometry_data, lam, nelement, target_order, upsampling_factor, expn_order)
            for tau in taus:
