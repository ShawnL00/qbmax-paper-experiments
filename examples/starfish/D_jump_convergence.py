...
def visualize_results(qbmax_results, qbx_results, lams, nelements, target_order, expn_order, upsampling_factor):
    ...
    for i, lam in enumerate(lams):
        qbx_errors = [np.max(qbx_results[lam]['abs_error'][j]) for j in range(len(nelements))]
        qbmax_errors = [np.max(qbmax_results[lam]['abs_error'][j]) for j in range(len(nelements))]
    ...
    plt.suptitle(r"Jump Relations of $\mathcal{D}_k\sigma$" + "\n" +
                 rf"$N = {nelements}, p={expn_order}$, $q={target_order + 1}$, $\kappa={upsampling_factor}$",
                 y=0.98)
...
for lam in lams:
    ...
    for nelement in nelements:
        (sources_h, targets_h, centers_in_h, centers_out_h, 
         weights_nodes_h, expansion_radii_h, normals_h, normals_target_h, hmax_h) = setup_geometry(
        ...
        (sources, targets, centers_in, centers_out, 
         weights_nodes, expansion_radii, normal, normal_target) = preprocess_arrays(
            sources_h, targets_h, centers_in_h, centers_out_h, 
            weights_nodes_h, expansion_radii_h, normals_h, normals_target_h
        )
        ...
        qbmax_error = np.abs(qbmax_result_lpot1 - qbmax_result_lpot2 + sigma_fun(targets_h))
        ...
        qbx_error = np.abs(qbx_result_lpot1 - qbx_result_lpot2 + sigma_fun(targets_h))
        
        qbx_results[lam]['abs_error'].append(qbx_error)
        
print("Test completed. Visualizing results...")
fig = visualize_results(qbmax_results, qbx_results, lams, nelements, 
