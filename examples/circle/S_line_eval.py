...
def preprocess_arrays(
    actx, sources, targets, centers_in, centers_out, weights_nodes, expansion_radii, normal
):
...
def analytical_solution(lam, r, tau, targets_h, m, side):
    ...
    for i in range(n_points):
        ...
        if side == -1:
            coeff = float(
                mpmath.besselk(m, lam) * mpmath.besseli(m, lam * (1 - (1 - tau) * r_i))
            )
        else:
            coeff = float(
                mpmath.besseli(m, lam) * mpmath.besselk(m, lam * (1 + (1 - tau) * r_i))
            )
...
fig = visualize_results(
    qbmax_results, qbx_results, lams, taus, nelement, expn_order, target_order, upsampling_factor
)
plt.show()