...
def reference_solution(lam, tau, targets_h, side, expansion_radii_h, normals_target_h):
    ...
    geometry_data_ref = setup_geometry(ref_element, expn_order_ref, target_order_ref, upsampling_factor_ref)
    ...
    asymexpn_ref = AsymptoticDividingLineTaylorExpansion(
        knl, asym_knl,
        expn_order_ref,
        tau = tau)
    
    from sumpy.qbx import LayerPotential
    lplot_asym = LayerPotential(actx.context, expansion = asymexpn_ref, source_kernels=(knl,), target_kernels=(knl,))

    _, (result_ref, ) = lplot_asym(actx.queue, 
...
def run_comparison(lams, taus, nelement=40, target_order=5, expn_order=5, upsampling_factor=4):
    ...
    for lam in lams:
        ...
        for tau in taus:
            utrue_vec_in = reference_solution(lam, tau, targets_h, -2, expansion_radii_h, normals_target_h)
            utrue_vec_out = reference_solution(lam, tau, targets_h, 2, expansion_radii_h, normals_target_h)
