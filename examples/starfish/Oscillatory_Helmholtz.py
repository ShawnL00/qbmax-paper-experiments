...
def preprocess(sources, targets, centers_in, centers_out, weights_nodes, expansion_radii, normal):
...
def visualize_results():
    ...
    plt.suptitle(
        rf"$N={nelement}$, $p={expn_order}$, $q={target_order+1}$, $\kappa={upsampling_factor}$",
        fontsize=14,
        y=0.98,
        x=0.52,
    )
...
y = radius * (1 + 1/4 * np.sin(5 * theta)) * np.array([np.cos(theta), np.sin(theta)])
...
(sources_h, targets_h, centers_in_h, centers_out_h, weights_nodes_h,
    expansion_radii_h, normals_h, normals_target_h, panel_h) = setup_geometry(
...
for k in k_values:
    ...
    for tau in taus:
        ...
        asymexpn_S = AsymptoticDividingLineTaylorExpansion(knl, asym_knl, expn_order, tau=tau)
        asymexpn_D = AsymptoticDividingLineTaylorExpansion(dknl, asym_knl, expn_order, tau=tau)
        ...
        _, (qbmax_S,) = lplot_S_asym(actx.queue, targets=targets, sources=sources,
                                    ...
                                    expansion_radii=expansion_radii, **S_extra_kwargs)
        ...
        _, (qbmax_D,) = lplot_D_asym(actx.queue, targets=targets, sources=sources,
                                    ...
                                    expansion_radii=expansion_radii, **D_extra_kwargs)

        _, (qbx_D,) = lplot_D_line(actx.queue, targets=targets, sources=sources,
                                    centers=centers_in, strengths=strength_D,
                                    expansion_radii=expansion_radii, **D_extra_kwargs)

        qbmax_S = actx.to_numpy(qbmax_S) * np.exp(-lam * (1 - tau) * expansion_radii_h)
        qbmax_D = actx.to_numpy(qbmax_D) * np.exp(-lam * (1 - tau) * expansion_radii_h)
...
print("Computation is completed. Creating plots...")
