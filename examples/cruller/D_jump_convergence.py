...
def setup_geometry(level, expn_order, target_order, upsampling_factor=1):
    ...
    normals_target_h = normals_target_h.reshape(ambient_dim, -1)
...
def generate_evaluation_targets(n_u=10, n_v=10, target_level=2):
    ...
    idx = 0
    for _j, v in enumerate(v_vals):
        for _i, u in enumerate(u_vals):
            pos, normal, radius = cruller_func(u, v, 4, 2, 20*target_level,
                                               10*target_level)
...
def main():
    ...
    for lam in lams:
        ...
        for level in nlevel:
            ...
            (sources_h, _, _, _, weights_nodes_h,
             _, normals_h, _, hmax_h) = setup_geometry(
