...
The analytical solution is given by:
    u_true(x) := S_k σ(x) = {
        e^{imθ_x} I_m(k|x|) K_m(k),     |x| ≤ 1
        e^{imθ_x} I_m(k) K_m(k|x|),     |x| > 1
    }
...
def setup_geometry(nelement, expn_order, target_order, upsampling_factor=1):
    ...
    from meshmode.mesh.generation import make_curve_mesh, circle
...
def preprocess_arrays(actx, sources, targets, centers_in, centers_out, weights_nodes, expansion_radii, normal):
    """Convert numpy arrays to device arrays."""
    return (
        actx.from_numpy(sources),
        actx.from_numpy(targets),
        actx.from_numpy(centers_in),
        actx.from_numpy(centers_out),
        actx.from_numpy(weights_nodes),
...
def analytical_solution(lam, r, tau, targets_h, m, side):
    ...
    for i in range(n_points):
        ...
        if side == -1:
            coeff = float(mpmath.besselk(m, lam) * 
                         mpmath.besseli(m, lam * (1 - (1 - tau) * r_i)))
        else:
            coeff = float(mpmath.besseli(m, lam) * 
                         mpmath.besselk(m, lam * (1 + (1 - tau) * r_i)))
...
def evaluate_expansion(expansion_type, knl, asym_knl, expn_order, tau, lam, 
                      actx, targets, sources, expansion_radii, centers_in, centers_out,
                      weights_nodes_h, sources_h, targets_h):

    from sumpy.expansion.local import AsymptoticDividingLineTaylorExpansion, LineTaylorLocalExpansion
...
def run_comparison(lams, taus, nelement=40, target_order=5, expn_order=5, upsampling_factor=4):
    ...
    (sources_h, targets_h, centers_in_h, centers_out_h, 
     weights_nodes_h, expansion_radii_h, normals_h, hmax_h) = geometry_data
    ...
    (sources, targets, centers_in, centers_out, weights_nodes, expansion_radii, normal) = processed_arrays
    ...
    for lam in lams:
        ...
        for tau in taus:
            utrue_vec_in = analytical_solution(lam, expansion_radii_h, tau, targets_h, m, -1)
            utrue_vec_out = analytical_solution(lam, expansion_radii_h, tau, targets_h, m, 1)

            qbmax_eval = evaluate_expansion(
                'QBMAX', knl, asym_knl, expn_order, tau, lam,
                actx, targets, sources, expansion_radii, centers_in, centers_out,
                weights_nodes_h, sources_h, targets_h
            )
            ...
            # QBX evaluation
            qbx_eval = evaluate_expansion(
                'QBX', knl, asym_knl, expn_order, tau, lam,
                actx, targets, sources, expansion_radii, centers_in, centers_out,
                weights_nodes_h, sources_h, targets_h
            )
...
def visualize_results(qbmax_results, qbx_results, lams, taus, 
    ...
    plt.rcParams.update({
        'text.usetex': True,
        'font.family': 'serif',        'text.latex.preamble': r'\usepackage{amsmath,amsfonts,lmodern}',
        'pgf.rcfonts': False,
        'pgf.texsystem': 'pdflatex',
        'axes.prop_cycle': cycler(color=['#002147', '#B22222', '#708090', '#228B22', '#6A0DAD'])
    })
    ...
    for i, lam in enumerate(lams):
        ...
        axs[0].loglog(plot_taus, qbx_in, marker=qbx_marker, linestyle='--', 
                      color=color, label=f'QBX, $k={lam}$', markersize=6)
        axs[0].loglog(plot_taus, qbmax_in, marker=qbmax_marker, linestyle='-', 
                      color=color, label=f'QBMAX, $k={lam}$', markersize=6)
        axs[1].loglog(plot_taus, qbx_out, marker=qbx_marker, linestyle='--', 
                      color=color, label=f'QBX, $k={lam}$', markersize=6)
        axs[1].loglog(plot_taus, qbmax_out, marker=qbmax_marker, linestyle='-', 
                      color=color, label=f'QBMAX, $k={lam}$', markersize=6)
    ...
    plt.suptitle(
        rf"Close Evaluation of $\mathcal{{S}}_{{k}}\sigma$: $N={nelement}$, "
...
fig = visualize_results(
    qbmax_results, qbx_results, lams, taus,
    nelement, expn_order, target_order, upsampling_factor
)
